// POST /api/internal/migrations/runs/:id/profile
...
export async function POST(
  ...
  try {
    ...
    await orchestrator.runPhase(id, "profile");

    const updated = await prisma.migrationRun.findUnique({
      where: { id },
      select: { status: true, sourceProfile: true },
    });

    return NextResponse.json({
      status: updated?.status,
      profile: updated?.sourceProfile ? JSON.parse(updated.sourceProfile) : null,
    });
  } catch (error) {
